...
async function initializeApp() {
  ...
  createBuildingBlocksBackground(config.gradientStart || '#72d0eb', config.gradientEnd || '#0084ad');
  setupEventListeners();
}
...
async function loadCompetitors() {
  if (config.competitors && config.competitors.length > 0) {
    ...
    await showAlert('No competitors found in configuration. Redirecting to setup page...', 'error', 'Setup Required');
    window.location.href = 'setup.html';
  }
}
...
async function generateAllocation() {
  ...
  if (!word1 || !word2 || !word3 || digit1 === '' || digit2 === '' || digit3 === '') {
    await showAlert('Please fill in all fields to generate the allocation', 'warning', 'Missing Information');
    return;
  }

  const letterRegex = /^[A-Za-z]+$/;
  if (!letterRegex.test(word1) || !letterRegex.test(word2) || !letterRegex.test(word3)) {
    await showAlert('Words must contain only letters (A-Z)', 'warning', 'Invalid Word Format');
    return;
  }
  ...
  if (num1 < 0 || num1 > 99 || num2 < 0 || num2 > 99 || num3 < 0 || num3 > 99) {
    await showAlert('Numbers must be between 0 and 99', 'warning', 'Invalid Number Range');
    return;
  }
...
async function saveSeed() {
  ...
  if (!seedDisplay || !seedDisplay.textContent) {
    await showAlert('No seed to save. Please generate an allocation first.', 'warning', 'No Seed');
    return;
  }
...
`;
  ...
  if ('showSaveFilePicker' in window) {
    try {
      ...
      await showAlert(`Allocation saved successfully as:<br><strong>${fileName}</strong>`, 'success', 'Allocation Saved');
    } catch (err) {
